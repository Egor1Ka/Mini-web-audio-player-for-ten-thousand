class AudioDriver {
  ...
  public async play(value?:number) {
    if (!this.audioBuffer) {
      throw new Error(
    ...
    const offset = value || this.pausedAt
    this.bufferSource.start(0, offset); // run the sound. pausedAt remembers a last pause time
  ...
  public async ChengeSoubndTime (value:number){
    await this.pause()
    this.play(value) 
  } 
}
